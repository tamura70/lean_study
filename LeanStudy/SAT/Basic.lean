...
/-- 3CNF -/
abbrev CNF.three (f : CNF α) := ∀ c ∈ f, c.length ≤ 3
...
theorem Literal.eval_negate (a : Assignment α) (x : Literal α) :
  ...
  obtain h1 | h2 := this
  case _ =>
    simp_all only [Bool.not_true, Bool.decide_eq_false, Bool.decide_eq_true]
  case _ =>
    simp_all only [Bool.not_false, Bool.decide_eq_true, Bool.decide_eq_false, Bool.not_not]
